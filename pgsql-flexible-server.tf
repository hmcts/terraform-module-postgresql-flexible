locals {
  ...
  db_reader_user  = var.reader_group_name != null ? var.reader_group_name : (local.is_prod ? "DTS JIT Access ${var.product} DB Reader SC" : "DTS ${upper(var.business_area)} DB Access Reader")
  ...
  use_legacy_jenkins_admin                = local.enable_aad_group_access && var.preserve_legacy_jenkins_admin && var.existing_admin_user_object_id == null && var.admin_user_object_id != null
