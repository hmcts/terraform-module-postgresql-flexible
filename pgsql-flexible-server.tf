...
resource "azurerm_postgresql_flexible_server" "pgsql_server" {
  ...
  delegated_subnet_id = var.public_access == true ? "" : var.pgsql_delegated_subnet_id == "" ? data.azurerm_subnet.pg_subnet[0].id : var.pgsql_delegated_subnet_id
  private_dns_zone_id = var.public_access == true ? "" : local.private_dns_zone_id
