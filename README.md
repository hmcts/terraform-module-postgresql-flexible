...
Declare service_criticality as a variable in variables.tf, defaulting to 1:
...
## Product-scoped reader access and identity migration

Set `reader_group_name` to an existing Entra group to isolate a product's
non-production access package from the shared business-area reader group.
The default is null and retains existing reader group naming. Writer access
is unaffected and remains opt-in.

The group must be deployed before applying this module. Changing the group
reruns the existing permission provisioner via its `db_reader_user` trigger;
it does not replace the server or database. Existing groups and their grants
are not revoked by changing this input. Removing historical access is a
separate, explicitly reviewed operation.

`preserve_legacy_jenkins_admin` is opt-in (false by default) so existing master
consumers retain their principal administrator. OPAL must set it to true when
moving from `DTSPO-30107-additional-postgres-admins` to preserve its current
administrator addresses and database ownership. `existing_admin_user_object_id`
can instead select an explicit existing principal; additional principals are
keyed by object ID and exclude the preserved principal.

Validate existing and future-table reads using the actual migration/table-owner
role. The existing default-privileges SQL runs as the provisioning identity;
this is not proof that every other table creator's defaults are configured.
...
| <a name="input_backup_vault_principal_id"></a> [backup\_vault\_principal\_id](#input\_backup\_vault\_principal\_id) | Principal ID of the backup vault's managed identity. Update if vault is recreated. | `string` | `"cfc3a379-b0fa-4a91-bdb1-51948bde431f"` | no |
...
| <a name="input_preserve_legacy_jenkins_admin"></a> [preserve\_legacy\_jenkins\_admin](#input\_preserve\_legacy\_jenkins\_admin) | Keep the legacy Jenkins PTL service principal as the stateful PostgreSQL Entra admin while adding admin\_user\_object\_id as an additional admin. Disable after database ownership has been reassigned from the legacy role. | `bool` | `false` | no |
...
| <a name="input_reader_group_name"></a> [reader\_group\_name](#input\_reader\_group\_name) | Optional Entra reader group display name. Null preserves the existing production product group and non-production business-area group. The group must already exist. | `string` | `null` | no |
